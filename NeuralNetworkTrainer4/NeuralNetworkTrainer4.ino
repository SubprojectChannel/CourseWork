//this one does the 8 outputs one
//
...
class Matrix {
  public:
    ...
    */
    ...
    /*
    void MatrixMultiply(Matrix* a, Matrix* b, String Mode) { //no nenory loss
      if (a->cols != b->rows) {
        Serial.println("Size mismatch in MatrixMultiply(Matrix* a, Matrix* b): " + a->info() + " " + b->info());
        while (true) {};
      }
      ...
      for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
          ...
          for (int k = 0; k < a->cols; k++) {
            ...
            sum += a->data[a_i][a_k] * b->data[b_k][b_j];
          }
          newData[i][j] = sum;
        }
      }
...
void train(Array* input_array, Array* desiredOutputs_array) {
      ...
      for (int i = numberOfLayers - 1; i > 0; i--) {
        ...
        if (i == numberOfLayers - 1) {
          ...
          //Matrix weights_i_T(&weights[i], "weights_i_T");
          //weights_i_T.transpose();
          //errors.MatrixMultiply(&weights_i_T, &previous_errors, "MxM");
          errors.MatrixMultiply(&weights[i], &previous_errors, "TxM");
          //weights_i_T.remove();
        }
        ...
        //neurons_im1_T.transpose();
        //changesToWeights.MatrixMultiply(&changesToBiases, &neurons_im1_T);
        changesToWeights.MatrixMultiply(&changesToBiases, &neurons[i-1], "MxT");
    ...
    */
    ...
    void fit(int acceptedIterationsGoal, float treshold, int printRate) {
